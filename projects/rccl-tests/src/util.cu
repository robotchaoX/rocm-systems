 ...
 ************************************************************************/
// This contains an utilities to handle output both to stdout and to
// json files.
//
...
//
// However, this means that the code is brittle to changes and care
...
static int parseRankInfo(rankInfo_t *rank, const char *instring) {
  int end;
  sscanf(instring,
         "#  Rank %d Group %d Pid %d on %1023s device %d [%127[^]]] %1023[^\n]\n%n",
         &rank->rank,
         &rank->group,
         &rank->pid,
         rank->hostname,
...
testResult_t writeDeviceReport(size_t *maxMem, int localRank, int proc, int totalProcs, int color, const char hostname[], const char *program_name) {
  PRINT("# rccl-tests version %s-%s rccl-headers=%d rccl-library=%d\n", NCCL_TESTS_VERSION, rcclTestsGitHash, NCCL_VERSION_CODE, test_ncclVersion);
