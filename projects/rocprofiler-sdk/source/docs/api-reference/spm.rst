...
2. To identify the counters supported by an agent, query the available counters with ``rocprofiler_iterate_agent_spm_supported_counters``. Here is an example of a single agent returning the available counters in ``gpu_counters``:
...
.. code-block:: cpp
    ...
    ROCPROFILER_CALL(rocprofiler_iterate_agent_spm_supported_counters(
                         agent,
...
.. code-block:: cpp
    ...
    void
    spm_sampling_callback(rocprofiler_context_id_t ctx,
                         ...
                         uint64_t drop_count)
    {
        for(size_t i = 0; i < num_headers; i++)
        {
            ...
            if(cur_header->category ==  ROCPROFILER_BUFFER_CATEGORY_COUNTERS)
            {
                if(cur_header->kind ==  ROCPROFILER_COUNTER_RECORD_PROFILE_COUNTING_DISPATCH_HEADE)
                {
                    //Process the header 
                    auto* header =
                       static_cast<rocprofiler_spm_dispatch_counting_service_data_t*>(header->payload);
                }
                else if(cur_header->category == ROCPROFILER_BUFFER_CATEGORY_COUNTERS)
                {  
...
.. note::
    A user can synchronously flush buffers via ``rocprofiler_buffer_flush`` that triggers ``spm_sampling_callback``.
